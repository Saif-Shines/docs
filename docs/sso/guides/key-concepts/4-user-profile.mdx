...
```js wordWrap=true title="Example of Normalized User Profile"
{
...
- The <SimpleCode>sub</SimpleCode> field combines the <SimpleCode>connection_id</SimpleCode> and the provider's unique identifier.
- The identities array may contain multiple objects if the user has authenticated through different methods.
- The <SimpleCode>provider_raw_attributes</SimpleCode> object contains all original data from the Identity Provider, which may vary based on the provider and connection type.
